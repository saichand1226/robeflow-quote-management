...
export async function loadQuote(supabase:any,id:number):Promise<any|null>{
 ...
 if(error)return null;return {...camel(quote),items:(items??[]).map(camel),payments:(payments??[]).map(camel),attachments:(attachments??[]).map(camel),activities:(activities??[]).map(camel)};
}
