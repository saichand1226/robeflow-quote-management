...
const documentOf=(row:any,currentId?:number)=>({id:row.id,version:versionOf(row.file_name),fileName:row.file_name.replace(/^\[Pick List v\d+ Uploaded\]\s*/i,""),size:Number(row.size)||0,source:"Uploaded",uploadedBy:"RobeFlow staff",isCurrent:row.id===(currentId??row.id),createdAt:row.created_at});
