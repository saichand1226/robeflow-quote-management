...
const intakeUrl =
  typeof window === "undefined"
    ? "/customer-intake"
    : `${window.location.origin}/customer-intake`;
...
export function CustomersPanel() {
  ...
  const filtered = useMemo(
    () =>
      customers.filter((c) =>
        `${c.name} ${c.companyName} ${c.email} ${c.phone} ${c.siteAddress}`
          .toLowerCase()
  ...
  async function archiveCustomer() {
    if (
      !selected ||
      !window.confirm(
        `Archive ${selected.name}? Their quotes will remain available.`,
      )
    )
      return;
    const response = await fetch("/api/customers", {
      ...
      <section className="flex flex-col gap-4 rounded-2xl border bg-white p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Customer contact details and installation addresses.
          </p>
        </div>
        ...
        <div className="grid gap-4 p-5 md:grid-cols-2 xl:grid-cols-3">
          {filtered.map((customer) => (
            <article
              ...
              className="cursor-pointer rounded-2xl border p-5 transition hover:border-emerald-500 hover:shadow-md"
            >
              ...
              <div className="mt-4 space-y-2 text-sm text-slate-600">
                ...
                {customer.phone && (
                  <Line icon={<Phone />}>{customer.phone}</Line>
                )}
                {customer.address && (
                  <Line icon={<MapPin />}>{customer.address}</Line>
                )}
        ...
        <DialogContent className="sm:max-w-xl">
          <form onSubmit={addCustomer}>
            <DialogHeader>
              ...
              <DialogDescription>
                Save their details now so they can be selected on future quotes.
              </DialogDescription>
            </DialogHeader>
          ...
          {selected && (
            <form onSubmit={saveCustomer}>
              <DialogHeader>
                ...
                <DialogDescription>
                  Edit customer details, notes and review their quotation
                  history.
                </DialogDescription>
              </DialogHeader>
              <div className="grid gap-4 py-5 sm:grid-cols-2">
                ...
                <FormField label="Site / installation address" wide>
                  <Input
                    name="siteAddress"
                    defaultValue={selected.siteAddress}
                  />
                </FormField>
...
export function EnquiriesPanel() {
  ...
  const filtered = useMemo(
    () =>
      enquiries.filter((e) =>
        `${e.name} ${e.companyName} ${e.projectType} ${e.areas} ${e.status}`
          .toLowerCase()
          .includes(search.toLowerCase()),
      ),
    [enquiries, search],
  ...
  async function updateEnquiry(
    enquiry: Enquiry,
    changes: { status?: string; salespersonName?: string },
  ) {
    ...
    if (response.ok) {
      ...
      setEnquiries((current) =>
        current.map((item) =>
          item.id === enquiry.id ? { ...item, ...result.enquiry } : item,
        ),
      );
    }
  }
  async function createQuote(enquiry: Enquiry) {
    ...
    try {
      ...
      setEnquiries((current) =>
        current.map((item) =>
          item.id === enquiry.id ? { ...item, status: "Quoted" } : item,
        ),
      );
      window.alert(
        `${result.quote.quoteNumber} created as a draft with the customer details and plans.`,
      );
    } finally {
      setConverting(null);
    }
      ...
      <section className="grid gap-5 rounded-2xl border bg-white p-5 shadow-sm lg:grid-cols-[1fr_auto] lg:items-center">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            New showroom enquiries appear here with their contact details,
          ...
          >
              ...
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-semibold text-emerald-800">
                    ENQ-{String(enquiry.id).padStart(4, "0")}
                  </span>
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  {enquiry.companyName || enquiry.projectType} ·{" "}
                  {new Date(enquiry.createdAt).toLocaleString("en-NZ")}
                </p>
              </div>
              <div className="flex flex-wrap gap-2">
                <select
                  value={enquiry.salespersonName || ""}
                  onChange={(e) =>
                    updateEnquiry(enquiry, { salespersonName: e.target.value })
                  }
                  className="h-9 rounded-lg border bg-white px-3 text-sm font-semibold"
                  aria-label={`Assign ${enquiry.name}`}
                >
                ...
                <select
                  ...
                  onChange={(e) =>
                    updateEnquiry(enquiry, { status: e.target.value })
                  }
                  className="h-9 rounded-lg border bg-white px-3 text-sm font-semibold"
                >
                  <option>New</option>
            ...
            <div className="mt-5 grid gap-5 lg:grid-cols-3">
              <div className="space-y-2 text-sm">
                ...
                {enquiry.siteAddress && (
                  <Line icon={<MapPin />}>{enquiry.siteAddress}</Line>
                )}
              </div>
                ...
                {enquiry.attachments.length > 0 && (
                  <div className="mt-3 flex flex-wrap gap-2">
                    {enquiry.attachments.map((file) => (
                      <a
                        key={file.id}
                        href={`/api/enquiry-attachments/${file.id}`}
                        className="inline-flex items-center gap-2 rounded-lg border px-3 py-2 font-medium hover:bg-slate-50"
                      >
            ...
            <div className="mt-4 flex justify-end">
              <Button
                disabled={
                  converting === enquiry.id || enquiry.status === "Quoted"
                }
                onClick={() => createQuote(enquiry)}
              >
...
export function InvoicesPanel() {
  const [invoices, setInvoices] = useState<InvoiceQuote[]>([]),
    ...
    [dueDates, setDueDates] = useState<Record<number, string>>({}),
  ...
  async function load() {
    ...
    setNumbers(
      Object.fromEntries(
        rows.map((q: InvoiceQuote) => {
          const balance =
            q.serviceType === "Installation" &&
            q.jobStage === "Completed" &&
            q.depositInvoiceSentAt &&
            !q.balanceInvoiceSentAt;
          return [
            q.id,
            balance
              ? q.balanceInvoiceNumber || suggestedInvoiceNumber(q, "balance")
              : q.depositInvoiceNumber ||
                q.invoiceNumber ||
                suggestedInvoiceNumber(
                  q,
                  q.serviceType === "Installation" ? "deposit" : "full",
                ),
          ];
        }),
      ),
    ...
    setNotes(
      Object.fromEntries(
        rows.map((q: InvoiceQuote) => [q.id, q.paymentNote ?? ""]),
      ),
    );
  }
  useEffect(() => {
  ...
  const filtered = useMemo(
    () =>
      invoices.filter(
        (q) =>
          (statusTab === "All" || q.invoiceStatus === statusTab) &&
          `${q.quoteNumber} ${displayedJobNumber(q)} ${q.customerName} ${q.companyName} ${q.project} ${q.invoiceNumber} ${q.salespersonName} ${q.invoiceStatus} ${q.paymentNote}`
            .toLowerCase()
            .includes(search.toLowerCase()),
      ),
  ...
  async function sendInvoice(quote: InvoiceQuote) {
    ...
    if (!quote.email) {
      window.alert(
        "Add the customer email address before sending the invoice.",
      );
      return;
    }
    setSaving(quote.id);
    try {
      ...
      await load();
      window.alert(
        `${phase === "deposit" ? "50% deposit" : phase === "balance" ? "Remaining balance" : "Full"} invoice ${numbers[quote.id]} was emailed to ${quote.email}.`,
      );
    } finally {
      setSaving(null);
    }
  ...
  async function sendOrderConfirmation(quote: InvoiceQuote) {
    if (!quote.email)
      return window.alert(
        "Add the customer email address before sending the order confirmation.",
      );
    setSaving(quote.id);
    try {
      const response = await fetch(`/api/orders/${quote.id}/email`, {
          method: "POST",
        }),
        result = await response.json();
      if (!response.ok)
        return window.alert(
          result.error || "Order confirmation could not be sent.",
        );
      await load();
  ...
  async function approveForDispatch(quote: InvoiceQuote) {
    ...
    try {
      ...
      if (!response.ok) {
        window.alert(
          result.error || "The dispatch approval could not be updated.",
        );
        return;
      }
      await load();
  ...
  const invoiced = invoices.filter(
      (q) => q.invoiceStatus && q.invoiceStatus !== "To be Invoiced",
    ),
    totalInvoiced = invoiced.reduce((sum, q) => sum + q.amount, 0),
    totalPaid = invoices.reduce((sum, q) => sum + Number(q.totalPaid || 0), 0),
    balanceAlerts = invoices.filter(
      (q) =>
        q.serviceType === "Installation" &&
        q.jobStage === "Completed" &&
        !!q.depositInvoiceSentAt &&
        !q.balanceInvoiceSentAt &&
        Number(q.totalPaid || 0) < Number(q.amount),
    );
  return (
    <div className="space-y-6">
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <MetricCard
          label="Outstanding"
          value={money(Math.max(0, totalInvoiced - totalPaid))}
          tone="amber"
        />
        <MetricCard
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="font-bold text-amber-950">
                {balanceAlerts.length} completed installation
                {balanceAlerts.length === 1 ? " needs" : "s need"} a remaining
                invoice
              </p>
              <p className="text-sm text-amber-800">
                Open the highlighted job below, enter the balance Xero invoice
                number, and send the remaining invoice.
              </p>
            </div>
            <Button
              variant="outline"
              ...
              onClick={() => {
                setStatusTab("All");
                setSearch(balanceAlerts[0].quoteNumber);
                setExpanded(balanceAlerts[0].id);
              }}
            >
              Review now
            </Button>
      ...
      <section className="overflow-hidden rounded-2xl border bg-white shadow-sm">
        <div className="flex flex-col gap-4 border-b p-5 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Create the invoice, mark it sent, then keep its payment status up
              to date.
            </p>
          </div>
          <div className="relative">
            ...
            <Input
              ...
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Search invoices or payment status…"
            />
          </div>
        </div>
        <div className="flex gap-2 overflow-x-auto border-b px-5 py-3">
          {[
            ...
            <Button
              key={name}
              variant={statusTab === name ? "default" : "outline"}
              size="sm"
              onClick={() => setStatusTab(name)}
            >
              {name}
              <span className="rounded-full bg-white/20 px-1.5 text-xs">
                {name === "All"
                  ? invoices.length
                  : invoices.filter((q) => q.invoiceStatus === name).length}
              </span>
            </Button>
          ))}
        </div>
        <div className="grid gap-4 p-5">
          {filtered.map((quote) => {
            ...
            const installation = quote.serviceType === "Installation",
              balanceDue =
                installation &&
                quote.jobStage === "Completed" &&
                !!quote.depositInvoiceSentAt &&
                !quote.balanceInvoiceSentAt &&
                Number(quote.totalPaid || 0) < Number(quote.amount),
              phaseSent = installation
                ? balanceDue
                  ? !!quote.balanceInvoiceSentAt
                  : !!quote.depositInvoiceSentAt
                : !!quote.invoiceSentAt,
            ...
            return (
              <article key={quote.id} className="rounded-2xl border p-5">
                ...
                >
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span
                        className={`rounded-full px-2.5 py-1 text-xs font-semibold ${status === "Paid in Full" ? "bg-emerald-100 text-emerald-800" : status === "To be Invoiced" ? "bg-amber-100 text-amber-800" : "bg-violet-100 text-violet-800"}`}
                      >
                        {status}
                      </span>
                      <span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold">
                        {quote.serviceType || "Pick Up"}
                      </span>
                      {quote.accountsApproved && (
                ...
                {open && (
                  <div className="mt-5 grid gap-4 rounded-xl border bg-slate-50 p-4 md:grid-cols-3">
                    <div>
                      ...
                      <p className="text-sm text-slate-500">
                        {money(Number(quote.servicePrice || 0))} service charge
                      </p>
                    </div>
                    <div>
                      <p className="text-xs font-bold uppercase text-slate-400">
                      ...
                      <p className="mt-1 font-semibold">
                        {money(quote.totalPaid || 0)} paid
                      </p>
                      <p className="text-sm text-slate-500">
                        {money(
                          Math.max(0, quote.amount - (quote.totalPaid || 0)),
                        )}{" "}
                        owing
                      </p>
                    </div>
                    <div className="flex flex-wrap items-end gap-2">
                      ...
                      {quote.invoiceNumber && (
                        <a
                          href={`/api/invoices/${quote.id}/pdf`}
                          className="inline-flex h-10 items-center gap-2 rounded-md border bg-white px-3 text-sm font-medium"
                        >
                          <Download className="size-4" />
                          Invoice PDF
                      ...
                      <Button
                        variant="outline"
                        disabled={
                          saving === quote.id || !!quote.orderConfirmationSentAt
                        }
                        onClick={() => sendOrderConfirmation(quote)}
                      >
                        <Mail className="size-4" />
                        {quote.orderConfirmationSentAt
                          ? "Order confirmation sent"
                          : "Email full order confirmation"}
                      </Button>
                    </div>
                  </div>
                )}
                <div className="mt-5 grid gap-3 border-t pt-4 lg:grid-cols-[1fr_12rem_1fr_auto] lg:items-end">
                  <div className="grid gap-2">
                    <Label htmlFor={`invoice-${quote.id}`}>
                      {installation
                        ? balanceDue
                          ? "Remaining balance Xero invoice number"
                          : "50% deposit Xero invoice number"
                        : "Xero invoice number"}
                    </Label>
                    <Input
                      id={`invoice-${quote.id}`}
                  ...
                  <div className="grid gap-2">
                    <Label htmlFor={`due-${quote.id}`}>Invoice due date</Label>
                    <Input
                      id={`due-${quote.id}`}
                      type="date"
                      value={dueDates[quote.id] || ""}
                      disabled={phaseSent}
                      onChange={(e) =>
                        setDueDates((current) => ({
                          ...current,
                  ...
                  {phaseSent ? (
                    <div className="flex gap-2">
                      <a
                        href={`/invoice/${quote.acceptanceToken}`}
                        target="_blank"
                        rel="noreferrer"
                        className="inline-flex h-10 items-center gap-2 rounded-md border bg-white px-4 text-sm font-medium hover:bg-slate-50"
                      >
                      ...
                      {!installation && (
                        <Button
                          ...
                          onClick={() => updateInvoice(quote, "To be Invoiced")}
                        >
                          Move back
                        </Button>
                      )}
                ...
                {quote.invoiceSentAt && (
                  <div className="mt-3 flex flex-col gap-3 rounded-xl border border-cyan-200 bg-cyan-50 p-3 sm:flex-row sm:items-center sm:justify-between">
                    <p className="text-sm text-cyan-900">
                      Invoice sent{" "}
                      {new Date(quote.invoiceSentAt).toLocaleString("en-NZ")}
                    </p>
                    <Button
                      variant={quote.accountsApproved ? "outline" : "default"}
                      disabled={saving === quote.id}
                      onClick={() => approveForDispatch(quote)}
                    >
                ...
                {sent && (
                  <details
                    className="mt-4 rounded-xl border bg-slate-50"
                    open={quote.payments?.length > 0}
                  >
                    ...
                    <div className="border-t p-4">
                      <div className="space-y-2">
                        {quote.payments?.map((payment) =>
                          editingPayment?.id === payment.id ? (
                            <form
                              key={payment.id}
                              onSubmit={(event) => editPayment(event, payment)}
                              className="grid gap-3 rounded-lg border border-emerald-300 bg-white p-3 md:grid-cols-[9rem_10rem_10rem_1fr_auto]"
                            >
                              <Input
                                name="amount"
                                required
                              ...
                              />
                              <div className="flex gap-2">
                                <Button disabled={saving === payment.id}>
                                  Save
                                </Button>
                                <Button
                                  type="button"
                      ...
                      {(quote.totalPaid || 0) < quote.amount && (
                        <form
                          onSubmit={(e) => recordPayment(e, quote)}
                          className="mt-4 grid gap-3 border-t pt-4 md:grid-cols-[9rem_10rem_10rem_1fr_auto]"
                        >
                          <Input
                            name="amount"
                            required={
                              (paymentModes[quote.id] || "Bank transfer") !==
                              "Account"
                            }
                            disabled={
                              (paymentModes[quote.id] || "Bank transfer") ===
                            ...
                            max={Math.max(
                              0,
                              quote.amount - (quote.totalPaid || 0),
                            )}
                            step="0.01"
                            placeholder={
                              (paymentModes[quote.id] || "Bank transfer") ===
                          ...
                          <select
                            name="method"
                            value={paymentModes[quote.id] || "Bank transfer"}
                            onChange={(event) =>
                              setPaymentModes((current) => ({
                                ...current,
...
export function TeamPanel() {
  ...
  async function addMember(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setTeam((current) =>
        [...current, result.member].sort((a, b) =>
    ...
    <div className="space-y-6">
      <section className="flex flex-col gap-4 rounded-2xl border bg-white p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Add salespeople so quotes and showroom enquiries can be assigned
            clearly.
          </p>
        </div>
        <Button onClick={() => setOpen(true)}>
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        <article className="rounded-2xl border bg-white p-5 shadow-sm">
          <div className="flex items-center gap-3">
            <span className="grid size-11 place-items-center rounded-full bg-emerald-100 font-bold text-emerald-800">
              S
            </span>
            <div>
        ...
        {team.map((member) => (
          <article
            key={member.id}
            className={`rounded-2xl border bg-white p-5 shadow-sm ${member.active ? "" : "opacity-60"}`}
          >
            <div className="flex items-center gap-3">
              <span className="grid size-11 place-items-center rounded-full bg-slate-100 font-bold">
        ...
        <DialogContent className="sm:max-w-lg">
          <form onSubmit={addMember} noValidate>
            <DialogHeader>
              ...
              <DialogDescription>
                They will appear in assignment menus across RobeFlow.
              </DialogDescription>
            </DialogHeader>
            <div className="grid gap-4 py-5 sm:grid-cols-2">
...
function QrCard() {
  ...
  return (
    <div className="flex items-center gap-4 rounded-2xl border bg-slate-50 p-3">
      <img
        src={qr}
        alt="QR code for customer quotation form"
        className="size-24 rounded-lg bg-white"
      />
      <div>
        ...
        <p className="mt-1 max-w-56 text-xs text-muted-foreground">
          Print this QR code or open the form on a showroom device.
        </p>
        <a
          href={intakeUrl}
          target="_blank"
