...
export default function QuoteDashboard({currentName,currentEmail,isAdmin,userRole="Staff"}:{currentName:string;currentEmail:string;isAdmin:boolean;userRole?:string}){
 ...
 const activeQuotes=useMemo(()=>quotes.filter(q=>!["Declined","Completed"].includes(q.status)&&q.jobStage!=="Sale Lost"&&!((q.jobStage==="Completed"||q.dispatchStatus==="Completed"||q.dispatchStatus==="Collected")&&q.invoiceStatus==="Paid in Full")),[quotes]);
...
<TableCell><span>{new Date(q.validUntil).toLocaleDateString("en-NZ",{day:"numeric",month:"short",year:"numeric"})}</span>{q.validUntil<new Date().toISOString().slice(0,10)&&!["Accepted","Declined","Completed"].includes(q.status)&&<span className="ml-2 inline-flex rounded-full bg-amber-100 px-2 py-1 text-xs font-bold text-amber-800">Follow up</span>}</TableCell>
...
function QuoteRecord({quote,open,setOpen,onUpdated,onDuplicated,settings}:{quote:Quote|null;open:boolean;setOpen:(v:boolean)=>void;onUpdated:(q:Quote)=>void;onDuplicated:(q:Quote)=>void;settings:CompanySettings|null}){
 ...
 async function emailFollowUp(){if(active.id<0)return;setBusy(true);try{const r=await fetch(`/api/quotes/${active.id}/follow-up`,{method:"POST"}),d=await r.json();if(!r.ok){window.alert(d.error||"The follow-up email could not be sent.");return}window.alert(`Follow-up email for ${active.quoteNumber} was sent to ${active.email}.`)}finally{setBusy(false)}}
...
{working.validUntil<new Date().toISOString().slice(0,10)&&!["Accepted","Declined","Completed"].includes(working.status)&&<Button variant="outline" className="border-amber-300 bg-amber-50 text-amber-900 hover:bg-amber-100" onClick={emailFollowUp} disabled={busy||working.id<0||!working.email}><Mail className="size-4"/>{busy?"Sending…":"Send follow-up email"}</Button>}
