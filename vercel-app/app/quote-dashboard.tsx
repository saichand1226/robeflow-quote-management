...
function printQuotation(
  ...
  if (!popup) {
    window.alert(
      "Please allow pop-ups for RobeFlow, then try Download PDF again.",
    );
    return;
  }
  const customerDetails = [
    ...
    .filter(Boolean)
    .map((value) => `<div>${printText(value)}</div>`)
    .join("");
  const rows =
    items
      .map((item) => {
        return `<tr><td><strong>${printText(item.category)}</strong><span>${printText(quoteItemDetails(item))}</span></td><td>${printText(money(Number(item.price)))}</td></tr>`;
      })
      .join("") +
    (Number(quote.servicePrice) > 0
      ? `<tr><td><strong>${printText(quote.serviceType || "Service")}</strong><span>Service charge</span></td><td>${printText(money(Number(quote.servicePrice)))}</td></tr>`
      : "");
  popup.document.open();
  popup.document
    .write(`<!doctype html><html><head><meta charset="utf-8"><title>${printText(quote.quoteNumber)} | Quotation</title><style>
...
export default function QuoteDashboard({
  ...
  const [quotes, setQuotes] = useState<Quote[]>([]),
    [customerCount, setCustomerCount] = useState(0),
    [companySettings, setCompanySettings] = useState<CompanySettings | null>(
      null,
    ),
    [team, setTeam] = useState<TeamMember[]>([]),
    [search, setSearch] = useState(""),
    [serviceFilter, setServiceFilter] = useState("All"),
    [salesFilter, setSalesFilter] = useState("All"),
    [selectedJobIds, setSelectedJobIds] = useState<number[]>([]),
    ...
    [statusBusy, setStatusBusy] = useState<number | null>(null),
    [selected, setSelected] = useState<Quote | null>(null),
    [recordOpen, setRecordOpen] = useState(false);
  useEffect(() => {
    fetch("/api/settings")
      .then((r) => (r.ok ? r.json() : null))
      .then((d) => setCompanySettings(d?.settings ?? null))
      .catch(() => {});
  }, []);
  useEffect(() => {
    if (section === "dashboard") {
      ...
      fetch("/api/customers")
        .then((r) => (r.ok ? r.json() : null))
        .then((d) => setCustomerCount(d?.customers?.length ?? 0))
        .catch(() => {});
      fetch("/api/team")
        .then((r) => r.json())
        .then((d) =>
          setTeam((d.team ?? []).filter((m: TeamMember) => m.active)),
        )
        .catch(() => {});
    }
  }, [section]);
  const activeQuotes = useMemo(
    () =>
      quotes.filter(
        (q) =>
          !["Declined", "Completed"].includes(q.status) &&
          q.jobStage !== "Sale Lost" &&
          !(
            (q.jobStage === "Completed" ||
              q.dispatchStatus === "Completed" ||
              q.dispatchStatus === "Collected") &&
            q.invoiceStatus === "Paid in Full"
          ),
      ),
    [quotes],
  );
    ...
    () =>
      activeQuotes.filter(
        (q) =>
          ...
          (salesFilter === "All" ||
            (q.salespersonName ?? "Sai Muddasani") === salesFilter) &&
          `${q.quoteNumber} ${displayedJobNumber(q)} ${q.customerName} ${q.project} ${q.status} ${q.serviceType ?? "Pick Up"} ${q.salespersonName ?? "Sai Muddasani"}`
            .toLowerCase()
            .includes(search.toLowerCase()),
      ),
    [activeQuotes, search, serviceFilter, salesFilter],
  );
  const pipeline = activeQuotes
    .filter((q) => ["Draft", "Sent", "Accepted"].includes(q.status))
    .reduce((s, q) => s + q.amount, 0);
  async function createQuote(payload: NewQuotePayload) {
    setSaving(true);
    try {
      ...
      setQuotes((c) => [d.quote, ...c]);
      return d.quote as Quote;
    } finally {
      setSaving(false);
    }
  }
  ...
  function addDuplicated(copy: Quote) {
  ...
  async function changeStatus(quote: Quote, status: string) {
    ...
    if (quote.id < 0) {
      setQuotes((current) =>
        current.map((q) => (q.id === quote.id ? { ...q, status } : q)),
      );
      return;
    }
    setStatusBusy(quote.id);
    try {
      const details = await fetch(`/api/quotes/${quote.id}`);
      if (!details.ok) throw Error();
      const data = await details.json();
      const updated = await fetch(`/api/quotes/${quote.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...data.quote, status }),
      });
      if (!updated.ok) throw Error();
      const result = await updated.json();
      replaceQuote(result.quote);
    } catch {
      window.alert("The quote status could not be updated. Please try again.");
    } finally {
      setStatusBusy(null);
    }
  }
  async function deleteSelectedJobs() {
    const selectedQuotes = quotes.filter(
      (q) => selectedJobIds.includes(q.id) && q.id > 0,
    );
    if (!selectedQuotes.length) return;
    if (
      !window.confirm(
        `Archive ${selectedQuotes.length} selected job${selectedQuotes.length === 1 ? "" : "s"}? You can restore them later from Archive.`,
      )
    )
      return;
    const results = await Promise.all(
      selectedQuotes.map((q) =>
        fetch(`/api/quotes/${q.id}`, { method: "DELETE" }),
      ),
    );
    const deleted = selectedQuotes
      .filter((_, index) => results[index].ok)
      .map((q) => q.id);
    setQuotes((current) => current.filter((q) => !deleted.includes(q.id)));
    setSelectedJobIds([]);
    if (deleted.length !== selectedQuotes.length)
      window.alert("Some selected jobs could not be archived.");
  }
  return (
    <div className="min-h-screen bg-background text-foreground">
      <aside
        className={`fixed inset-y-0 left-0 z-40 w-64 bg-sidebar text-sidebar-foreground transition-transform lg:translate-x-0 ${nav ? "translate-x-0" : "-translate-x-full"}`}
      >
        ...
        <nav className="space-y-2 p-4">
          ...
          {["Admin", "Accounts", "Operations", "Staff"].includes(userRole) && (
            <Nav
          ...
          {["Admin", "Operations", "Staff"].includes(userRole) && (
            <Nav
              active={section === "operations"}
              icon={<BriefcaseBusiness />}
              label="Jobs & installation"
              onClick={() => {
          ...
          {["Admin", "Sales", "Operations", "Staff"].includes(userRole) && (
            <Nav
              active={section === "lost"}
              icon={<X />}
              label="Sale lost"
        ...
        <div className="absolute inset-x-4 bottom-4 rounded-xl border border-sidebar-border bg-sidebar-accent p-4">
          ...
          <form action="/auth/signout" method="post">
            <button
              type="submit"
              className="mt-3 flex items-center gap-2 text-xs font-semibold text-white hover:text-emerald-300"
            >
              Log out
            </button>
          </form>
      ...
      <main className="lg:ml-64">
            ...
            <div>
              ...
              <p className="hidden text-sm text-muted-foreground sm:block">
                {section === "dashboard"
                  ...
                  : section === "customers"
                    ? "Keep customer details ready for future quotations."
                    : section === "invoices"
                      ? "Accepted jobs ready for the accounts team."
                      : section === "team"
                        ? "Manage salespeople and job assignments."
                        : section === "accounts"
                          ? "Review new Robeflow staff registrations."
                          : "Review new requests and prepare their quotations."}
              </p>
            </div>
          </div>
          {section === "dashboard" &&
        ...
        <div className="space-y-7 p-4 sm:p-8">
          {section === "board" ? (
            <JobBoard currentName={currentName} />
          ) : section === "customers" ? (
            <CustomersPanel />
          ) : section === "todo" ? (
            <EnquiriesPanel />
            ...
            <ClosedJobsPanel kind="completed" />
          ) : section === "lost" ? (
            <ClosedJobsPanel kind="lost" />
          ) : section === "products" ? (
            <ProductsPanel />
          ) : section === "team" ? (
            <TeamPanel />
            ...
            <>
              ...
              <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                ...
                <Metric
                  ...
                  note="Excludes completed and lost"
                />
                <Metric
                  label="Accepted"
                  value={String(
                ...
                <div className="flex flex-col gap-4 border-b p-5 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Select jobs to archive, or click a quote to view, edit,
                      duplicate or download it.
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    {selectedJobIds.length > 0 && (
                      ...
                      >
                        ...
                        Archive selected ({selectedJobIds.length})
                      </Button>
                    )}
                  ...
                  <div className="flex gap-2 overflow-x-auto">
                    {[
                      ...
                      <button
                        ...
                        className={`whitespace-nowrap rounded-full px-4 py-2 text-sm font-semibold ${serviceFilter === service ? "bg-slate-900 text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}
                      >
                        {service}
                      </button>
                    ))}
                    ...
                    <TableRow>
                      <TableHead className="w-12 pl-5">
                        <input
                          ...
                          checked={
                            filtered.filter((q) => q.id > 0).length > 0 &&
                            filtered
                              .filter((q) => q.id > 0)
                              .every((q) => selectedJobIds.includes(q.id))
                          }
                          onChange={(e) =>
                            setSelectedJobIds(
                              e.target.checked
                      ...
                      >
                        ...
                        >
                          <input
                            type="checkbox"
                            disabled={q.id < 0}
                            aria-label={`Select ${q.quoteNumber}`}
                        ...
                        <TableCell>
                          <span className="font-medium">
                            {q.serviceType ?? "Pick Up"}
                          </span>
                          {Number(q.servicePrice) > 0 && (
                            <span className="block text-xs text-muted-foreground">
                              {money(Number(q.servicePrice))}
                            </span>
                          )}
                        </TableCell>
                        <TableCell>
                          ...
                          {q.validUntil <
                            ...
                            !["Accepted", "Declined", "Completed"].includes(
                              ...
                              <span className="ml-2 inline-flex rounded-full bg-amber-100 px-2 py-1 text-xs font-bold text-amber-800">
                                Follow up
                              </span>
                            )}
                        </TableCell>
                        ...
                        >
                          <div className="flex flex-wrap items-center gap-2">
                            <select
                              ...
                              className={`h-9 rounded-lg border px-2 text-sm font-semibold ${statusStyle(q.status)}`}
                            ...
                            {q.status === "Accepted" && (
                              <span
                                className={`rounded-full px-2.5 py-1 text-xs font-bold ${q.invoiceStatus === "Paid in Full" ? "bg-emerald-100 text-emerald-800" : q.invoiceStatus && q.invoiceStatus !== "To invoice" ? "bg-violet-100 text-violet-800" : "bg-orange-100 text-orange-800"}`}
                              >
                                {q.invoiceStatus === "Manual message"
                                  ? q.paymentNote || "Manual message"
                                  : q.invoiceStatus &&
                                      q.invoiceStatus !== "To invoice" &&
                                      q.invoiceStatus !== "Invoice sent"
                                    ? q.invoiceStatus
                                    : q.invoiceStatus === "Invoice sent"
                                      ? "Waiting for payment"
                                      : "Invoice pending"}
      ...
      <QuoteRecord
        ...
        onUpdated={replaceQuote}
        onDuplicated={addDuplicated}
...
function Nav({
  ...
  return (
    <button
      onClick={onClick}
      className={`flex w-full items-center gap-3 rounded-xl px-4 py-3 text-left text-sm font-medium ${active ? "bg-sidebar-primary text-slate-950" : "text-slate-300 hover:bg-sidebar-accent hover:text-white"}`}
    >
...
function NewQuote({
  ...
  createQuote: (p: NewQuotePayload) => Promise<Quote | undefined>;
  saving: boolean;
  onEmailed: (quote: Quote) => void;
}) {
  const initialItems = () =>
    [
      "Bedroom 1",
  ...
  const [items, setItems] = useState<QuoteItem[]>(initialItems),
    [customerId, setCustomerId] = useState<number | undefined>(),
    ...
    [serviceType, setServiceType] = useState("Installation"),
    [servicePrice, setServicePrice] = useState(""),
    [discountPercent, setDiscountPercent] = useState("0"),
    [salespersonName, setSalespersonName] = useState("Sai Muddasani"),
  ...
  const subtotal = roundMoney(
    ...
    discount = Math.min(100, Math.max(0, Number(discountPercent) || 0)),
    total = roundMoney(subtotal * (1 - discount / 100));
  function updateItem(
    index: number,
    key:
      | "category"
    ...
    setItems((current) =>
      current.map((item, i) => {
        ...
        const next = {
          ...item,
          [key]: value,
          ...(key === "systemType" ? { colour: "Undecided" } : {}),
        };
        return {
          ...next,
          price: roundMoney(
            (Number(next.quantity) || 1) * (Number(next.unitPrice) || 0),
          ),
        };
      }),
    );
  }
  function setCatalogPrice(index: number, value: number) {
    updateItem(index, "unitPrice", String(value));
  }
  function duplicateItem(index: number) {
    setItems((current) => [
      ...current.slice(0, index + 1),
      { ...current[index], category: `${current[index].category} copy` },
  ...
  function reset() {
    ...
    setSalespersonName("Sai Muddasani");
    setValidUntil("");
    setFollowUpDate("");
    setFiles([]);
  ...
  async function submit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const sendAfterSave = submitter?.value === "email";
    if (!customerName) {
      setCustomerOpen(true);
      return;
    }
    if (sendAfterSave && !email.trim()) {
      window.alert(
        "Add the customer's email address before saving and emailing the quote.",
      );
      setCustomerOpen(true);
      return;
    }
    const quote = await createQuote({
      ...
      items: items
        ...
        .map((i) => ({
          ...
          colour: i.colour,
          designSelection: i.designSelection,
          hardwareColour: i.hardwareColour,
          doorConfiguration: i.doorConfiguration,
          mirrorOption: i.mirrorOption,
          frameTrackColour: i.frameTrackColour,
    ...
    try {
      for (const file of files) {
        ...
        const upload = await fetch(`/api/quotes/${quote.id}/attachments`, {
          method: "POST",
          body: data,
        });
      ...
      if (pickListFile) {
        const data = new FormData();
        data.append("file", pickListFile);
        const upload = await fetch(`/api/pick-list/${quote.id}/documents`, {
          method: "POST",
          body: data,
        });
        if (!upload.ok) {
          ...
          throw new Error(
            result.error || `Could not upload ${pickListFile.name}.`,
          );
        }
      }
      if (sendAfterSave) {
        const sent = await fetch(`/api/quotes/${quote.id}/email`, {
          method: "POST",
        });
        ...
        if (!sent.ok) {
          ...
          reset();
          window.alert(
            `Quote ${quote.quoteNumber} was saved as Draft, but the email was not sent: ${result.error || "Please try again from the saved quote."}`,
          );
          return;
        }
        onEmailed(result.quote);
        window.alert(
          `Quote ${quote.quoteNumber} was emailed to ${email}. The internal pick list was not sent.`,
        );
      } else window.alert(`Quote ${quote.quoteNumber} was saved as Draft.`);
      setOpen(false);
      reset();
    } catch (error) {
      setOpen(false);
      reset();
      window.alert(
        `The quote was saved, but ${error instanceof Error ? error.message : "the files or email could not be completed."}`,
      );
    } finally {
      setUploading(false);
    }
    ...
    <>
        ...
        <DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-2xl">
          <form onSubmit={submit}>
            <DialogHeader>
              ...
              <DialogDescription>
                Add the customer, prices, customer design files and the internal
                Excel pick list.
              </DialogDescription>
            </DialogHeader>
            <div className="grid gap-4 py-5">
              <button
                ...
                className="flex w-full items-center justify-between rounded-xl border bg-emerald-50/50 p-4 text-left transition hover:border-emerald-500"
              >
                <span className="flex items-center gap-3">
                  ...
                  <span>
                    ...
                    <span className="text-sm text-muted-foreground">
                      {customerName
                        ? companyName || email || phone || "Customer selected"
              ...
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  placeholder="e.g. New home wardrobes"
                  required
                />
              </div>
              <div className="grid gap-2">
              ...
              <div className="overflow-hidden rounded-2xl border bg-slate-50/70">
                <div className="flex items-center justify-between border-b bg-white px-4 py-3">
                  <div>
                    <p className="font-bold text-slate-900">Areas & products</p>
                    <p className="text-xs text-muted-foreground">
                      Name each room or storage area, then select its wardrobe
                      system.
                    </p>
                  </div>
                ...
                <div className="space-y-4 p-3 sm:p-4">
                  {items.map((item, index) => (
                    <div
                      key={index}
                      className="rounded-2xl border bg-white p-4 shadow-sm ring-1 ring-slate-100"
                    >
                      <div className="mb-4 flex items-center justify-between">
                        <span className="grid size-8 place-items-center rounded-full bg-slate-900 text-sm font-bold text-white">
                          {index + 1}
                        </span>
                        <div className="flex">
                          <Button
                            ...
                            aria-label={`Duplicate ${item.category}`}
                          >
                          ...
                          <Button
                            ...
                            onClick={() => {
                              if (
                                window.confirm(
                                  `Remove ${item.category} from this quote?`,
                                )
                              )
                                setItems((current) =>
                      ...
                      <div className="grid gap-3 sm:grid-cols-2">
                        <div className="grid gap-1.5">
                          ...
                          <Input
                            ...
                            placeholder="e.g. Bedroom 1, Storage 2"
                        ...
                        <div className="grid gap-1.5">
                          ...
                          <select
                            ...
                            onChange={(e) => {
                              ...
                              updateItem(index, "systemType", value);
                              const product = products.find(
                                (p) => p.name === value,
                              );
                              if (product)
                                setCatalogPrice(index, product.price);
                            }}
                            className="h-10 rounded-md border border-input bg-background px-3 text-sm"
                      ...
                      <ItemOptions
                        ...
                        onChange={(key, value) => {
                          ...
                          if (key === "designSelection") {
                            const product = products.find(
                              (p) => p.name === value,
                            );
                            if (product) setCatalogPrice(index, product.price);
                          }
                        }}
                      />
                      <div className="mt-3 grid gap-3 sm:grid-cols-[6rem_1fr_1fr]">
                        <div className="grid gap-1.5">
                          <Label
                        ...
                        <div className="grid gap-1.5">
                          ...
                          <div className="flex h-10 items-center justify-end rounded-md bg-slate-900 px-3 text-sm font-bold text-white">
                      ...
                      <Input
                        ...
                        placeholder="Optional measurements or internal description"
                      />
                      {coloursForSystem(item.systemType).length > 0 && (
                        <ColourPicker
                          systemType={item.systemType}
                          value={item.colour}
                          onChange={(value) =>
                            updateItem(index, "colour", value)
                          }
                        />
                      )}
                    </div>
              ...
              <div className="rounded-xl border bg-slate-50 p-4">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="grid gap-2">
                    <Label htmlFor="serviceType">Service option</Label>
                    <select
                      id="serviceType"
                      value={serviceType}
                      onChange={(e) => {
                        ...
                        if (product) setServicePrice(String(product.price));
                      }}
                      className="h-10 rounded-md border border-input bg-background px-3 text-sm"
                    >
              ...
              <div className="grid gap-4 rounded-xl border bg-amber-50/50 p-4 sm:grid-cols-2 sm:items-end">
                ...
                <div className="text-right">
                  ...
                  <p className="font-semibold text-amber-800">
                    Discount {discount}% · −{money(subtotal - total)}
                  </p>
                  <p className="mt-1 text-xl font-bold">Total {money(total)}</p>
                </div>
              </div>
              ...
              <div className="rounded-xl border border-emerald-200 bg-emerald-50/60 p-4">
                <div className="flex items-center gap-2">
                  ...
                  <Label htmlFor="pick-list-file">
                    Internal Excel pick list
                  </Label>
                </div>
                <p className="mt-1 text-xs leading-5 text-muted-foreground">
                  Fill this workbook manually in Excel. It stays internal for
                  Sales and Dispatch and is never sent to the customer.
                </p>
                <Input
                  id="pick-list-file"
                  ...
                  onChange={(e) => setPickListFile(e.target.files?.[0] ?? null)}
                />
                {pickListFile && (
                  <p className="mt-2 truncate text-sm font-medium text-emerald-800">
                    Selected: {pickListFile.name}
                  </p>
                )}
              </div>
              <div className="grid gap-2">
            ...
            <DialogFooter>
              ...
              <Button
                ...
                value="save"
                variant="outline"
                disabled={saving || uploading || !items.length}
              >
                {saving || uploading
                  ? "Saving…"
                  : `Save quote · ${money(total)}`}
              </Button>
              <Button
                type="submit"
                name="intent"
                ...
                disabled={saving || uploading || !items.length}
              >
                {saving || uploading ? "Preparing…" : "Save & email quote"}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
      ...
      <Dialog open={customerOpen} onOpenChange={setCustomerOpen}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>
              {addingCustomer ? "Add a new customer" : "Select customer"}
            </DialogTitle>
            <DialogDescription>
              {addingCustomer
                ? "This customer will be saved for future quotes."
                : "Search the customer database or add someone new."}
            </DialogDescription>
          </DialogHeader>
          {!addingCustomer ? (
            <div className="py-3">
                ...
                {customers
                  ...
                  .map((customer) => (
                    <button
                      ...
                      className="flex w-full items-start gap-3 rounded-xl border p-3 text-left hover:border-emerald-500 hover:bg-emerald-50"
                    >
            ...
            <div className="grid gap-4 py-3">
              ...
              <CustomerField
                ...
                placeholder="Optional company or builder"
              />
              <CustomerField
                icon={<Mail />}
                label="Email address"
              ...
              <CustomerField
                ...
                placeholder="Postal or billing address"
              />
              <CustomerField
                icon={<MapPin />}
                label="Site address"
                value={siteAddress}
                setValue={setSiteAddress}
  ...
  return (
    <div className="grid gap-2">
      <Label htmlFor={id} className="flex items-center gap-2">
        <span className="[&>svg]:size-4 text-primary">{icon}</span>
        {label}
      </Label>
      <Input
        id={id}
  ...
  return (
    <div className="mt-3">
      ...
      <div className="flex flex-wrap gap-2">
        {coloursForSystem(systemType).map((name) => (
          <button
            key={name}
            type="button"
            onClick={() => onChange(name)}
            className={`flex items-center gap-2 rounded-full border px-3 py-1.5 text-xs font-semibold transition ${value === name ? "border-primary ring-2 ring-primary/20" : "hover:border-slate-400"}`}
            style={{
              backgroundColor: colourSwatch(name),
              color: name === "Grey Ash" ? "#172033" : "#3f3729",
            }}
          >
            <span className="size-3 rounded-full border border-black/15 bg-white/40" />
            {name}
          </button>
        ))}
      </div>
    ...
    <div className="mt-3 grid gap-3 rounded-xl border border-slate-200 bg-slate-50 p-3 sm:grid-cols-2">
      ...
      {accessory && (
        <div className="grid gap-1.5 sm:col-span-2">
          ...
          <select
            required
            value={item.designSelection || ""}
            onChange={(e) => onChange("designSelection", e.target.value)}
            className="h-10 rounded-md border bg-white px-3 text-sm"
          >
            <option value="">Select an accessory</option>
            {ACCESSORY_OPTIONS.map((value) => (
              <option key={value}>{value}</option>
            ))}
          </select>
        </div>
      ...
      {sliding && (
        <>
          <div className="grid gap-1.5">
            <Label className="text-xs">Door configuration</Label>
            <select
              value={item.doorConfiguration || ""}
              onChange={(e) => onChange("doorConfiguration", e.target.value)}
              className="h-10 rounded-md border bg-white px-3 text-sm"
            >
              <option value="">Select configuration</option>
              {DOOR_CONFIGURATIONS.map((value) => (
                <option key={value}>{value}</option>
              ))}
            </select>
          </div>
          <div className="grid gap-1.5">
            <Label className="text-xs">Mirror option (optional)</Label>
            <select
              value={item.mirrorOption || "No mirror selected"}
              onChange={(e) => onChange("mirrorOption", e.target.value)}
              className="h-10 rounded-md border bg-white px-3 text-sm"
            >
              {MIRROR_OPTIONS.map((value) => (
                <option key={value}>{value}</option>
              ))}
            </select>
          </div>
          <div className="grid gap-1.5 sm:col-span-2">
            <Label className="text-xs">Frames & tracks</Label>
            <select
              value={item.frameTrackColour || "Undecided"}
              onChange={(e) => onChange("frameTrackColour", e.target.value)}
              className="h-10 rounded-md border bg-white px-3 text-sm"
            >
              {FRAME_TRACK_COLOURS.map((value) => (
                <option key={value}>{value}</option>
              ))}
            </select>
          </div>
  ...
  return (
    <div className="rounded-xl border border-dashed bg-muted/20 p-4">
      ...
      <p className="mt-1 text-xs text-muted-foreground">
        PDF, JPG, PNG, DWG or DXF · maximum 15 MB per file
      </p>
      <Input
        id="design-files"
        className="mt-3 bg-white"
...
function QuoteRecord({
  ...
  onDuplicated: (q: Quote) => void;
  settings: CompanySettings | null;
}) {
  const [editing, setEditing] = useState(false),
    [working, setWorking] = useState<Quote | null>(null),
    [team, setTeam] = useState<TeamMember[]>([]),
  ...
  const subtotal =
      ...
      Number(working.servicePrice ?? 0),
    discount = Math.min(100, Math.max(0, Number(working.discountPercent) || 0));
              ...
              return {
                ...
                price: roundMoney(
                  (Number(next.quantity) || 1) *
                    (Number(next.unitPrice) || Number(item.price) || 0),
                ),
              };
            }),
          }
  ...
  async function save() {
    ...
    try {
      ...
      if (!r.ok) throw Error();
      const refreshed = await fetch(`/api/quotes/${active.id}`),
        d = await refreshed.json();
      setWorking(d.quote);
      onUpdated(d.quote);
      setEditing(false);
    } finally {
  ...
  async function duplicate() {
    ...
    try {
      ...
      const created = await r.json(),
        details = await fetch(`/api/quotes/${created.quote.id}`),
        d = await details.json();
      setWorking(d.quote);
      onDuplicated(d.quote);
      setEditing(true);
    } finally {
      setBusy(false);
    }
  ...
  async function uploadAttachments(files: FileList | null) {
    if (!files?.length || active.id < 0) return;
    setBusy(true);
    try {
      ...
      for (const file of Array.from(files)) {
        const data = new FormData();
        data.append("file", file);
        const r = await fetch(`/api/quotes/${active.id}/attachments`, {
          method: "POST",
          body: data,
        });
        if (r.ok) {
          const d = await r.json();
          added.push(d.attachment);
        }
      }
      setWorking((current) =>
        current
  ...
  async function removeAttachment(id: number) {
    if (!confirm("Remove this design attachment?")) return;
  ...
  async function emailQuote() {
    ...
    setBusy(true);
    try {
      const r = await fetch(`/api/quotes/${active.id}/email`, {
        method: "POST",
      });
      const d = await r.json();
      if (!r.ok) {
        window.alert(d.error || "The email could not be sent.");
        return;
      }
      setWorking(d.quote);
      onUpdated(d.quote);
      window.alert(
        `Quote ${active.quoteNumber} was emailed to ${active.email}.`,
      );
    } finally {
      setBusy(false);
    }
  }
  async function emailFollowUp() {
    if (active.id < 0) return;
    setBusy(true);
    try {
      const r = await fetch(`/api/quotes/${active.id}/follow-up`, {
          method: "POST",
        }),
        d = await r.json();
      if (!r.ok) {
        window.alert(d.error || "The follow-up email could not be sent.");
        return;
      }
      window.alert(
        `Follow-up email for ${active.quoteNumber} was sent to ${active.email}.`,
      );
    } finally {
      setBusy(false);
    }
        ...
        <div className="quote-print-sheet rounded-sm bg-white text-slate-800">
            ...
            <div className="text-right">
              ...
              {working.emailedAt && (
                <p className="mt-1 text-xs text-slate-500">
                  Emailed {new Date(working.emailedAt).toLocaleString("en-NZ")}
                </p>
              )}
            </div>
          </div>
          <div className="grid gap-6 py-6 sm:grid-cols-2">
            <div>
              ...
              {editing ? (
                ...
                <>
                  ...
                  {working.companyName && (
                    <p className="text-sm font-medium">{working.companyName}</p>
                  )}
                  {working.email && (
                  ...
                  {working.phone && (
                    <p className="text-sm text-slate-500">{working.phone}</p>
                  )}
                  {working.customerAddress && (
            ...
            <div>
              <div className="grid grid-cols-2 gap-4 text-sm sm:text-right">
                ...
                <div>
                  ...
                  {editing ? (
                    ...
                    <p className="font-semibold">
                      {new Date(working.validUntil).toLocaleDateString("en-NZ")}
                    </p>
                  )}
                </div>
              </div>
              {editing ? (
                ...
                working.siteAddress && (
                  <div className="mt-5 text-sm sm:text-right">
          ...
          <div className="mb-5 rounded-xl border bg-slate-50 p-4">
            {editing ? (
              <div className="grid gap-2">
                ...
                <select
                  value={working.salespersonName ?? "Sai Muddasani"}
                  onChange={(e) =>
                    updateField("salespersonName", e.target.value)
                  }
              ...
              <div>
                ...
                <p className="mt-1 font-semibold">
                  {working.salespersonName ?? "Sai Muddasani"}
                </p>
              </div>
            )}
          </div>
          ...
          <div className="overflow-hidden rounded-xl border border-slate-200">
            ...
            {items.map((item, index) => (
              <div
                key={index}
                className="grid grid-cols-[1fr_9rem] items-start border-t border-slate-100 px-4 py-3 first:border-0"
              >
                <div>
                  {editing ? (
                      ...
                      <div className="grid gap-2 sm:grid-cols-[6rem_9rem_1fr]">
                        ...
                        <div>
                          ...
                          <Input
                            ...
                            placeholder="Measurements or notes"
                          />
                        </div>
                      </div>
                    </div>
          ...
          <div className="mt-3 grid gap-3 rounded-xl border border-emerald-200 bg-emerald-50/50 p-4 sm:grid-cols-[1fr_10rem] sm:items-end">
            {editing ? (
              <>
                <div className="grid gap-2">
                  <Label>Service option</Label>
                  <select
                    value={working.serviceType ?? "Pick Up"}
                ...
                <div className="grid gap-2">
                  ...
                  <Input
                    ...
                    min="0"
                    step="0.01"
                    value={working.servicePrice ?? 0}
                    onChange={(e) =>
                      updateField("servicePrice", e.target.value)
                    }
                  />
                </div>
              </>
              ...
              <>
                <div>
                  ...
                  <p className="mt-1 font-semibold">
                    {working.serviceType ?? "Pick Up"}
                  </p>
                </div>
                <p className="text-right font-semibold">
                  {money(Number(working.servicePrice ?? 0))}
                </p>
              </>
            )}
          </div>
              ...
              {editing ? (
                <>
                  ...
                  <Input
                    ...
                    max="100"
                    step="0.01"
                    value={working.discountPercent ?? 0}
                    onChange={(e) =>
                ...
                <>
                  ...
                  <p className="mt-1 font-semibold">{discount}%</p>
                </>
              )}
            </div>
          ...
          <div className="mt-3 grid gap-3 rounded-xl border bg-slate-50 p-4 sm:grid-cols-2">
            ...
            <div>
              {editing ? (
                ...
                <>
                  ...
                  <p className="mt-1 font-semibold">
                    {working.followUpDate
                      ? new Date(working.followUpDate).toLocaleDateString(
                          "en-NZ",
                        )
                      : "Not scheduled"}
                  </p>
            ...
            {working.customerComment && (
              <div>
                <p className="text-xs font-bold uppercase tracking-wider text-slate-400">
                  Customer comment
                </p>
                <p className="mt-1 text-sm">{working.customerComment}</p>
              </div>
            )}
          </div>
          {editing && (
          ...
          {working.id > 0 && (
            <div className="no-print mt-7 flex flex-col justify-between gap-3 rounded-xl border border-emerald-200 bg-emerald-50/60 p-4 sm:flex-row sm:items-center">
              <div>
                <h3 className="flex items-center gap-2 font-semibold">
                  <File className="size-4 text-emerald-700" />
                  Excel pick list
                </h3>
                <p className="mt-1 text-xs text-slate-600">
                  Create the customer-filled workbook, complete custom designs
                  in Excel, and upload the finished file to this quote.
                </p>
              </div>
              <a
                href={`/pick-list/${working.id}`}
                target="_blank"
                className="inline-flex h-10 shrink-0 items-center justify-center rounded-md bg-emerald-700 px-4 text-sm font-semibold text-white"
              >
                Open Excel pick list
              </a>
            </div>
          )}
          <div className="no-print mt-5 rounded-xl border bg-slate-50 p-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-xs text-slate-500">
                  Plans, drawings and design files saved with this quote.
                </p>
              </div>
              {working.id > 0 && (
                <Label className="cursor-pointer rounded-md bg-white px-3 py-2 shadow-sm ring-1 ring-slate-200 hover:bg-slate-50">
              ...
              <div className="mt-3 grid gap-2 sm:grid-cols-2">
                {working.attachments.map((file) => (
                  ...
                  >
                    ...
                    <a
                      className="min-w-0 flex-1 truncate text-sm font-medium hover:text-emerald-700 hover:underline"
          ...
          <div className="no-print mt-5 overflow-hidden rounded-xl border bg-white">
            <div className="flex items-center justify-between border-b bg-slate-50 px-4 py-3">
              <div>
                <h3 className="font-semibold">Quote revision history</h3>
                <p className="text-xs text-slate-500">
                  A permanent comparison of every quotation version.
                </p>
              </div>
              <span className="rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-bold text-emerald-800">
                {working.revisions?.length ?? 1} version
                {(working.revisions?.length ?? 1) === 1 ? "" : "s"}
              </span>
            </div>
              ...
              {(
                ...
                .map((version) => (
                  <details
                    ...
                    open={version.id === working.id}
                  >
                    <summary className="flex cursor-pointer list-none items-center justify-between gap-3">
                      <div>
                        <p className="font-semibold">
                          Revision {version.revision} · {version.quoteNumber}
                        </p>
                        <p className="text-xs text-slate-500">
                          {new Date(version.createdAt).toLocaleString("en-NZ")}{" "}
                          · {version.status}
                        </p>
                      </div>
                      <strong>{money(version.amount)}</strong>
                    </summary>
                    <ul className="mt-3 space-y-1.5 border-l-2 border-emerald-600 pl-3">
          ...
          {working.activities?.length ? (
            <div className="no-print mt-5 rounded-xl border bg-white p-4">
                ...
                {working.activities
                  ...
                  .map((activity) => (
                    ...
                    >
                      ...
                      <p className="text-xs text-slate-500">
                        {activity.detail} · {activity.actor} ·{" "}
                        {new Date(activity.createdAt).toLocaleString("en-NZ")}
                      </p>
                    </div>
                  ))}
              </div>
          ...
          <div className="mt-10 border-t pt-5 text-xs leading-5 text-slate-500">
            <p>
              Thank you for the opportunity to provide this quotation. Prices
              are in New Zealand dollars and include GST. This quotation remains
              valid until the date shown above.
            </p>
          ...
          <div className="flex w-full flex-wrap justify-between gap-2">
            <div className="flex gap-2">
              ...
              <Button
                variant="outline"
                onClick={() =>
                  printQuotation(working, items, total, gst, settings)
                }
              >
              ...
              <Button
                variant="outline"
                onClick={emailQuote}
                disabled={busy || working.id < 0 || !working.email}
              >
                <Mail className="size-4" />
                {working.emailedAt ? "Resend email" : "Email quote"}
              </Button>
              {working.validUntil < new Date().toISOString().slice(0, 10) &&
                !["Accepted", "Declined", "Completed"].includes(
                  working.status,
                ) && (
                  <Button
