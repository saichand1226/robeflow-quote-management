...
import { Archive, Building2, BriefcaseBusiness, Columns3, Copy, Download, File, FileText, LayoutDashboard, Mail, MapPin, Menu, PackagePlus, Paperclip, Pencil, Phone, Plus, ReceiptText, Search, Settings, Trash2, Truck, UserRound, Users, WalletCards, X } from "lucide-react";
...
<a href="/signout-with-chatgpt?return_to=%2F" target="_top" className="mt-3 flex items-center gap-2 text-xs font-semibold text-white hover:text-emerald-300">Log out</a>
</div>
</aside>
...
<DialogDescription>Add the customer, category prices and design files.</DialogDescription>
</DialogHeader>
...
function QuoteRecord({quote,open,setOpen,onUpdated,onDuplicated,settings}:{quote:Quote|null;open:boolean;setOpen:(v:boolean)=>void;onUpdated:(q:Quote)=>void;onDuplicated:(q:Quote)=>void;settings:CompanySettings|null}){
 ...
 async function save(){if(working.id<0){setEditing(false);return}setBusy(true);try{const r=await fetch(`/api/quotes/${working.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify(working)});if(!r.ok)throw Error();const refreshed=await fetch(`/api/quotes/${working.id}`),d=await refreshed.json();setWorking(d.quote);onUpdated(d.quote);setEditing(false)}finally{setBusy(false)}}
 async function duplicate(){if(working.id<0){return}setBusy(true);try{const r=await fetch(`/api/quotes/${working.id}`,{method:"POST"});if(!r.ok)throw Error();const created=await r.json(),details=await fetch(`/api/quotes/${created.quote.id}`),d=await details.json();setWorking(d.quote);onDuplicated(d.quote);setEditing(true)}finally{setBusy(false)}}
 async function uploadAttachments(files:FileList|null){if(!files?.length||working.id<0)return;setBusy(true);try{const added:Attachment[]=[];for(const file of Array.from(files)){const data=new FormData();data.append("file",file);const r=await fetch(`/api/quotes/${working.id}/attachments`,{method:"POST",body:data});if(r.ok){const d=await r.json();added.push(d.attachment)}}setWorking(current=>current?{...current,attachments:[...(current.attachments??[]),...added]}:current)}finally{setBusy(false)}}
 ...
 async function emailQuote(){if(working.id<0)return;setBusy(true);try{const r=await fetch(`/api/quotes/${working.id}/email`,{method:"POST"});const d=await r.json();if(!r.ok){window.alert(d.error||"The email could not be sent.");return}setWorking(d.quote);onUpdated(d.quote);window.alert(`Quote ${working.quoteNumber} was emailed to ${working.email}.`)}finally{setBusy(false)}}
